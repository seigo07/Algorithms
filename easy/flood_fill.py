# DFSを使用して、指定された座標 (r, c) から連結した領域（上、下、左、右）を再帰的に探索し塗りつぶす
# 時間・空間計算量: O(mn) m = 行数, n = 列数 最悪全マス見る キューに最大で全マス近く入る可能性がある
def flood_fill(image, sr, sc, color):

    rows = len(image)
    cols = len(image[0])
    start_color = image[sr][sc] # 元の色
    ...
    def dfs(r, c):
        ...
        # (r, c)がindex範囲を超えず、image[r][c]の色が start_color である場合にのみ処理を実行
        if 0 <= r < rows and 0 <= c < cols and image[r][c] == start_color:

            image[r][c] = color # 現在のピクセルを color に変更
